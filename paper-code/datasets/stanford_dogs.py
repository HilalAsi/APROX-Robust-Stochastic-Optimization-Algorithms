...
# train_dataset, test_dataset = Stanford_dogs()
def Stanford_dogs(root='./data', train=True, download=True, augment=True,
            randomize_labels=False, randomize_label_seed=42, batch_size = 4, num_workers = 4):
    input_size = 224
    input_transforms = transforms.Compose([
        transforms.RandomResizedCrop(input_size, ratio=(1, 1.3)), #use transforms.RandomSizedCrop(input_size) if this doesn't work
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor()])

    train_dataset = dogs(root="./data/Stanford-dogs/",
                         train=True,
                         cropped=False,
                         transform=input_transforms,
                         download=True)
    test_dataset = dogs(root="./data/Stanford-dogs/",
                        train=False,
                        cropped=False,
                        transform=input_transforms,
                        download=True)
    ...
    dataloaders['val'] = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size,
                                                       shuffle=True, num_workers=num_workers)
