...
class SGDWithTruncation(Optimizer):
    ...
    Example:
        >>> optimizer = SGDWithTruncation(model.parameters(), lr=0.1)
        >>> optimizer.zero_grad()
		>>> def closure():
    ...
    def __init__(self, params, lr=0.1, momentum=0, dampening=0,
        ...
        super(SGDWithTruncation, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(SGDWithTruncation, self).__setstate__(state)
...
class TruncatedAdagrad(Optimizer):
    ...
    def step(self, closure=None):
        ...
        if grad_normalized_norm>0:
            trunc_lr =  1.0 * float(loss.item()) / grad_normalized_norm
        for group in self.param_groups:
            for p in group['params']:
                if p.grad is None:
                    continue
